...
select "userId",
    ...
    where "username" = $1;
  `;
  // pg Node library expects the query method's second argument (if it exists) to be an array of values
  const newUsername = [username];
  db.query(sql, newUsername)
    .then(result => {
      const newUser = result.rows[0];
      if (!newUser) {
        throw new ClientError(401, 'invalid login');
      } else {
        argon2.verify(newUser.hashedPassword, password)
          .then(passwordMatch => {
            if (!passwordMatch) {
              throw new ClientError(401, 'invalid login');
            } else {
